...
const MOCK_GARAGES: ServiceProvider[] = [
  ...
  // Add more garages if needed for filtering
];
...
const INITIAL_MOCK_REQUESTS: ServiceRequest[] = [
  {
    ...
    requestTime: new Date(Date.now() - 1000 * 60 * 15), // 15 mins ago
    status: 'Pending',
    userName: 'Aisha K.',
  ...
  },
  {
    ...
    requestTime: new Date(Date.now() - 1000 * 60 * 45), // 45 mins ago
    status: 'Accepted',
    userName: 'John B.',
  ...
  },
  {
  ...
  },
   {
    ...
    selectedProvider: MOCK_GARAGES[0],
    requestTime: new Date(Date.now() - 1000 * 60 * 5), // 5 mins ago
    status: 'Pending',
    userName: 'David S.',
...
export default function GarageAdminPage() {
  ...
  const refreshData = () => {
    ...
    // For now, just reset to initial mock or a slightly modified version
    setRequests([...INITIAL_MOCK_REQUESTS.map(r => ({...r, requestTime: new Date(r.requestTime.getTime() + 1000)}))]); // slightly update time
    ...
    <div className="flex-grow flex flex-col p-4 md:p-6 space-y-6">
      <Card className="shadow-md">
        <CardHeader>
          ...
          <CardDescription>View and manage incoming roadside assistance requests.</CardDescription>
        </CardHeader>
